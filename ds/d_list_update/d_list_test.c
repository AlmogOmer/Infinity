...
int main() {
	...
	iter2 = DListIterPrev(DListEnd(list));
...
void testDoubly()
{
	...
	DListInsert(DListBegin(list), &a);/*a,b,c*/

	iter = DListIterNext(DListBegin(list));
	iter = DListIterNext(iter);
...
void testDoubly2()
{
	...
	if(!(*(int*)DListIterGetData(DListIterPrev(iter)) == 5))
	{
		printf("fail in %d\n", __LINE__);
	}
	...
	if(!(*(int*)DListIterGetData(iter2) == 450))
	{
		printf("fail in %d\n", __LINE__);
	}
	iter2 = DListIterNext(iter2);
